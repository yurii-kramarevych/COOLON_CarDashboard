...
apt-get install make build-essential libclang-dev ninja-build gcc git bison \
...
DIR="/usr/local/QtUbuntu/include/QtCore"
...
if [ -d "$DIR" ]; then
  ...
  echo "qtbase is exist ${DIR}..."
	...
	tar xf $qtbaseXZ
	cd $HOME/$qtbaseDir
	./configure -opengl es2 -opensource -confirm-license -force-debug-info -prefix /usr/local/QtUbuntu -nomake examples -nomake tests -v |& tee -a $OUTPUT_FILE
	make -j8
	make install
	export QT_SELECT=5
	cd ..
...
/usr/local/QtUbuntu/bin/qmake |& tee -a $OUTPUT_FILE
